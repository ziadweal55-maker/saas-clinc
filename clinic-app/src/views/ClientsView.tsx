    ...
    if (window.api && window.api.createClient) {
      ...
      if (res.success) {
        ...
        if ((window as any).showToast) {
          (window as any).showToast(t('toast_patient_created'), 'success');
        }
      } else {
        if ((window as any).showToast) {
          (window as any).showToast(t('toast_patient_error') + res.error, 'error');
        } else {
          alert(t('toast_patient_error') + res.error);
        }
      }
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-foreground font-heading tracking-tight italic">{t('patient_directory')}</h1>
          <p className="text-muted-foreground text-xs md:text-sm font-medium mt-1">{t('manage_patient_records')}</p>
        </div>
        ...
        <button 
          ...
          {showAddForm ? <><X size={18} /> {t('cancel')}</> : <><UserPlus size={18} /> {t('add_patient')}</>}
        </button>
      </div>
      ...
      {showAddForm && (
        <div className="bg-card rounded-2xl border border-border shadow-xl p-6 md:p-8 mb-6 md:mb-8 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-lg md:text-xl font-bold text-foreground font-heading">{t('new_patient_enrollment')}</h2>
          </div>
          
          <form onSubmit={handleCreateClient} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('first_name')}</label>
                <input 
                  required 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('last_name')}</label>
                <input 
                  required 
                  type="text" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('phone_number')}</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('age')}</label>
                <input 
                  type="number" 
                  min="0"
                  placeholder="25"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('address')}</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('how_did_you_know_us')}</label>
                <input 
                  type="text" 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('primary_clinical_notes')}</label>
              <textarea 
                rows={4} 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ms-1">{t('profile_type')}</label>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="px-4 md:px-5 pt-4 md:pt-5 flex flex-wrap gap-2">
          {[
            { id: 'all', label: t('all_patients'), color: 'text-foreground' },
            { id: 'physical_therapy', label: t('physical_therapy'), color: 'text-blue-600' },
            { id: 'nutrition', label: t('nutrition'), color: 'text-emerald-600' },
            { id: 'lymphatic', label: t('lymphatic'), color: 'text-purple-600' },
          ].map(tab => (
            <button
              key={tab.id}
        ...
        <div className="p-4 md:p-5 border-b border-border flex flex-col md:flex-row justify-between items-center gap-4 bg-muted/20">
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              className={`w-full ${isAr ? 'pr-11 pl-4' : 'pl-11 pr-4'} py-2.5 rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all bg-background text-foreground font-medium`}
            />
          </div>
          
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div className="flex items-center gap-2 w-full md:w-auto">
              <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground whitespace-nowrap">{t('sort')}</span>
              <select 
                value={sortBy} 
        ...
        <div className="overflow-x-auto">
          {filteredClients.length === 0 ? (
            <div className="p-12 md:p-20 text-center flex flex-col items-center">
              ...
              <h3 className="text-lg md:text-xl font-bold text-foreground mb-2 font-heading">{t('no_results_matched')}</h3>
              <p className="text-muted-foreground max-w-xs mx-auto text-xs md:text-sm font-medium leading-relaxed px-4">
                {searchTerm ? t('search_results_empty').replace('{searchTerm}', searchTerm) : t('directory_empty')}
              </p>
            </div>
          ) : (
            <>
              ...
              <table className="w-full text-start border-collapse hidden md:table">
                <thead>
                  <tr className="bg-muted/30 border-b border-border">
                    <th className="px-8 py-5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-start">{t('patient_details')}</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-start">{t('contact_info')}</th>
                    <th className={`px-8 py-5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest ${isAr ? 'text-left' : 'text-right'}`}>{t('reference_id')}</th>
                  </tr>
                </thead>
                  ...
                  {sortedClients.map((c, i) => (
                    <tr key={c.id || i} onClick={() => onSelectClient(c)} className="hover:bg-primary/5 cursor-pointer transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            {c.is_active === 0 && <span className="text-[9px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-bold uppercase mt-1 inline-block">{t('archived')}</span>}
                            {c.profile_types && c.profile_types.split(',').map((type: string) => {
                              ...
                              return (
                                <span key={type} className={`text-[8px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wide mt-0.5 me-1 inline-block ${colors[type as keyof typeof colors] || 'bg-muted text-foreground'}`}>
                      ...
                      <td className="px-8 py-5">
                        ...
                        <div className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter mt-0.5">{t('primary_contact')}</div>
                      </td>
                  ...
                  <div key={c.id || i} onClick={() => onSelectClient(c)} className="p-4 active:bg-primary/5 transition-colors group flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {c.is_active === 0 && <span className="text-[8px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-black uppercase mt-1 inline-block tracking-tighter">{t('archived')}</span>}
                      </div>
                    </div>
